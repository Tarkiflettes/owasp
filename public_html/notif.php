    ...
    if(isset($_SESSION['id']))
  {
	...
	<div id="hauteur">
          <form action="sendnotif.php" method="POST">
            <div id="formulaire">
              <h1>envoyer une notif</h1>
              <label>Titre</label>
              <input type="text" name="titre" id="champs" value="<?php echo $donnees['nomasso'];?>" required autofocus />
              <label>Message</label>
              <td colspan="2"><textarea name="message" cols="50" rows="10" id ="champs1"></textarea></td>
              <?php
              if($_SESSION['id'] == -1)
                {
                ?>
              ...
              <input type="submit" id="inscrire1" value="envoyer"/>
           </form> 
        </div>
    <?php
}
