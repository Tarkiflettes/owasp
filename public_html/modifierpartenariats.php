...
<?php
      if (isset($_SESSION['id']))
      {
        ...
        if(isset($_POST['nom']) AND !empty($_POST['nom']) && isset($_POST['contenu']) AND !empty($_POST['contenu'])) {
          ...
          $req = $db->prepare("UPDATE partenariat SET nom = :nom, description = :contenu, photo
          = :image WHERE id = :id");
        ...
        if(isset($_POST['idsuppr']) AND !empty($_POST['idsuppr']))
        {
          ...
          $req = $db->prepare("DELETE FROM partenariat WHERE id = :id");
      ...
      closedir($repertoire); // Ne pas oublier de fermer le dossier ***EN DEHORS de la boucle*** ! Ce qui évitera à PHP beaucoup de calculs et des problèmes liés à l'ouverture du dossier.
        ...
        $req = $db->prepare("SELECT * FROM partenariat WHERE id = :id ORDER BY id DESC");
        ...
        <div id="hauteur">
          <form action="modifierpartenariats.php" method="POST">
            <div id="formulaire">
              <h1>modifier</h1>
              <label>nom</label>
              <input type="text" name="nom" id="champs" value="<?php echo $News['nom'];?>" required autofocus />
              <label>contenu</label>
              <td colspan="2"><textarea name="contenu" cols="50" rows="2" id ="champs1"> <?php echo $News['description'];?> </textarea></td>
              <label>image</label>
                  <select name="image1" id="champ">
                    <option value="<?php echo $News['photo'];?>"><?php echo $News['photo'];?></option> 
                    <?php 
              ...
              <input type="hidden" name="id" value="<?php echo $News['id'];?>" id="champs" />
           ...
           <form action="modifierpartenariats.php" method="POST">
              ...
              <input type="hidden" name="idsuppr" value="<?php echo $News['id'];?>" id="champs" />
              <input type="hidden" name="image" value="<?php echo $News['photo'];?>" id="champs" />
